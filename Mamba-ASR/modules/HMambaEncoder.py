...
class RoutingModuleOutput:
    ...
    boundary_hard: Optional[torch.Tensor] = None  # (B, L, 2) - Gumbel-softmax hard output (training only, has STE gradients)
...
class RoutingModule(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            boundary_hard_out = boundary_hard  # Store for ratio_loss gradient flow
        ...
        return RoutingModuleOutput(
            ...
            boundary_hard=boundary_hard_out,
        )
...
def load_balancing_loss(router_output: RoutingModuleOutput, N: float) -> torch.Tensor:
    ...
    # =========================================================================
    # Use boundary_hard (Gumbel-softmax output with STE gradients) instead of
    # boundary_mask (boolean, no gradients). boundary_hard[..., 1] is the
    # differentiable "boundary" channel from hard Gumbel-softmax.
    boundary_hard = router_output.boundary_hard
    if boundary_hard is not None:
        # Training: use STE-differentiable hard samples
        actual_ratio = boundary_hard[..., 1].mean()
    else:
        # Inference: fall back to boolean mask (no gradients needed)
