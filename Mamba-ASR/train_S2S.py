...
class ASR(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            self.loss_ctc = loss_ctc.detach()
            self.loss_seq = loss_seq.detach()
    ...
    def fit_batch(self, batch):
        """Wrap fit_batch with OOM protection: skip batch and free memory on CUDA OOM."""
        try:
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.TRAIN:
            stage_stats["loss_ctc"] = float(self.loss_ctc)
            stage_stats["loss_seq"] = float(self.loss_seq)
