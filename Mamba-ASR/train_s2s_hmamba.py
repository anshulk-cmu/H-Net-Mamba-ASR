...
class HMambaASR(sb.core.Brain):
    ...
    def on_fit_start(self):
        ...
        if hasattr(self, 'optimizer') and hasattr(self, 'hmamba_encoder'):
            ...
            if not dc_in_opt:
                ...
                self.optimizer.add_param_group({'params': dc_params})
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # Total loss
        dc_loss_weight = getattr(self.hparams, 'hmamba_dc_loss_weight', 0.03) if stage == sb.Stage.TRAIN else 0.0
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            # Update early stopper with validation ACC (higher = better)
            if hasattr(self.hparams.epoch_counter, 'update_metric'):
                self.hparams.epoch_counter.update_metric(stage_stats["ACC"])
    ...
    def on_evaluate_start(self, max_key=None, min_key=None):
        ...
        # Ensure H-Mamba wrapper is initialized BEFORE loading checkpoints,
        # otherwise load_state_dict will fail (checkpoint has DC params but
        # model still has unwrapped ConMamba encoder)
        if not hasattr(self, '_hmamba_initialized'):
            self._wrap_encoder_with_hmamba()
            self._hmamba_initialized = True
