...
Core grid: **{Type A, Type B} × {N=1,2,3,4} × {Small, Large}**, and each trained model is then **decoded** along **{CTC, AED, CTC+AED} × {greedy, beam} × {no-LM, +LM}** (the decoding protocol, §6.3). The three decoders are **read-outs of one hybrid model**, not three trainings — CTC and AED heads come from the same run (§4.4), so a single training fills an entire decode sub-block.

**Committed run plan (decided 2026-07-19; supersedes the earlier screening→promotion staging).** The **full 16-cell hybrid grid** is trained — no Large-promotion pruning — plus **4 fixed-stride-pooling controls at Small** (Type A at $N{=}2,3,4$; Type B at $N{=}4$ only — Type B pools at stride $\sqrt N$, so its fixed variant exists only at perfect-square $N$; $N{=}1$ fixed is exactly the dynamic identity, no extra cell) plus **2 vocabulary-ablation runs** (the best Small config and the best Large config retrained at $V{=}750$, decoded with the 750-vocab LM) = **22 training runs**. Ordering: (1) a short hybrid dry-run gate (N=2 Small, few epochs, β active) checks the production training mode mechanically; (2) **A·Small·N=1 trains first and serves as the go/no-go gate** — its CTC-greedy test-clean WER must beat the sanity bar (< 12; no separate CTC-only training); (3) **all remaining Type A cells train next (Small then Large, plus the Type A controls); the Type B block (8 hybrid cells + the B·N4 control) proceeds after a review of the Type A results**; (4) the two V=750 runs launch once each size tier's winner is known. Each run is read out under the full decode matrix below (~10 meaningful WER numbers per model per test set).
...
- **Compute-bounded ordering.** The full 16-cell hybrid grid is committed, but Type B trains only after a review of the Type A results (§6.1); if that review descopes Type B cells, the two-stage-hierarchy conclusions (H5) would rest on a reduced grid.
